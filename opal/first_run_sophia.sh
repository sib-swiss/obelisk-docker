...
sed s/@host@/$POSTGRESDATA_HOST/g /opt/opal/data/test_resource | \
      sed s/@user@/$POSTGRESDATA_USER/g | \
      sed s/@password@/$POSTGRESDATA_PASSWORD/g | \
opal rest -o https://localhost:8443 -u administrator -p $OPAL_ADMINISTRATOR_PASSWORD -m POST /project/test/resources --content-type "application/json"
...
opal rest -o https://localhost:8443 --user administrator --password  $OPAL_ADMINISTRATOR_PASSWORD --method POST "/project/sophia/permissions/resources?type=USER&permission=RESOURCES_VIEW&principal=guest"
opal rest -o https://localhost:8443 --user administrator --password  $OPAL_ADMINISTRATOR_PASSWORD --method POST "/project/test/permissions/resources?type=USER&permission=RESOURCES_VIEW&principal=guest"
